...
bool Box::CheckBoxCorners(const Box& a_box, glm::vec2& a_contact, int& a_numOfContacts, float& a_pen, glm::vec2& a_edgeNormal)
{
	float minX =0.f, maxX=0.f, minY=0.f, maxY=0.f;
	float boxW = a_box.GetExtents().x * 2;
	float boxH = a_box.GetExtents().y * 2;
	int numLocalContcts = 0;
	...
	for (float x = -a_box.GetExtents().x; x < boxW; x += boxW)
	{
		for (float y = -a_box.GetExtents().y; y < boxH; y += boxH)
		{
			...
			//Update the extents in each cardianl direction of our box's space
	...
	if (maxX <= -m_extents.x || minX >= m_extents.x ||
		...
		return false;
	if (numLocalContcts == 0)
		return false;

	bool res = false;
	a_contact += m_position + (localContact.x * m_localX + localContact.y * m_localY) / (float)numLocalContcts;
	a_numOfContacts++;
