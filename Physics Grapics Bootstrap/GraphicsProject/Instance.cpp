...
void Instance::Draw(Scene* a_scene)
{
	...
	auto pvm = a_scene->GetCurrentCamera()->GetProjectionMatrix(a_scene->GetWindowSize().x,
		a_scene->GetWindowSize().y) * a_scene->GetCurrentCamera()->GetViewMatrix() * m_transform;
	//bind the transform
	m_shader->bindUniform("ProjectionViewModel", pvm);
	m_shader->bindUniform("CameraPosition", a_scene->GetCurrentCamera()->GetPosition());
	...
	//m_shader->bindUniform("LightColor1", m_light1.color);
	//m_shader->bindUniform("LightDirection1", m_light1.direction);
...
glm::mat4 Instance::MakeTransform(glm::vec3 a_position, glm::vec3 a_eulerAngles, glm::vec3 a_scale)
{
	return glm::translate(glm::mat4(1), a_position)
		...
		* glm::rotate(glm::mat4(1), glm::radians(a_eulerAngles.z), glm::vec3(1, 0, 0))
